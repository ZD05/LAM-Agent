...
class DesktopLauncher:
    ...
    def launch_file(self, file_info: Dict[str, Any]) -> Dict[str, Any]:
        """启动文件或快捷方式"""
        try:
            ...
            if file_type == 'shortcut':
                return self._launch_shortcut(file_info)
            else:
                return self._launch_file(file_info)
    ...
    def _launch_shortcut(self, shortcut_info: Dict[str, Any]) -> Dict[str, Any]:
        """启动快捷方式"""
        try:
            ...
            process = subprocess.Popen(
                ...
                cwd=working_dir,
                shell=True,
                creationflags=subprocess.CREATE_NEW_CONSOLE if shortcut_info.get('executable') else 0
            )
    ...
    def _launch_file(self, file_info: Dict[str, Any]) -> Dict[str, Any]:
        """启动文件"""
        try:
    ...
    def get_file_info(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        if file_path.lower().endswith('.lnk'):
            return self._analyze_shortcut(file_path)
        else:
...
def main():
    ...
    print(f"找到 {len(files)} 个可启动的文件/快捷方式:")
    print("-" * 60)
    ...
    for i, file_info in enumerate(files, 1):
        ...
        if file_info.get('target_path'):
            print(f"    目标: {file_info['target_path']}")
        print()
    
    # 搜索测试
    print("搜索测试 (关键词: 'python'):")
    python_files = launcher.search_files('python')
    print(f"找到 {len(python_files)} 个匹配的文件")
